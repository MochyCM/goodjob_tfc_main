...
func hello(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	fmt.Fprintf(w, "Modificado por Ramón/impact006 y anteriormente por patowc, modificación de modificación\nPara proyecto TFT\n")
	fmt.Fprintf(w, "=============================================================================\n")
	fmt.Fprintf(w, "=============================================================================\n")
	...
	for idx, row := range csv_data {
		if idx == 0 {
			...
			//Como el array está en string
			//fmt.Fprintf(w, row[0]+"         |"+row[1]+"   |"+row[2]+"     |"+row[3]+"     |"+"£\n") //un \n es un salto de línea
			//Finalmente, lo dejaré con símbolos para adecuarse mejor al formato, también se puede implementar la librería
			//tabwriter de Golang para dejarlo formateado y más bonito
			fmt.Fprintf(w, "   FECHA         $        ¥        CHF      £\n") //un \n es un salto de línea
		...
		row_year := row[0]
		if strings.ContainsAny(row_year, ";") { //busca si contiene un ;
			row_year = strings.ReplaceAll(row_year, ";", ",") //reemplaza en el string/cadena de texto de la variable
			//row_year, de un ; a un espacio
			row = strings.Split(row_year, ",") //Lo cortamos con un split y metemos en el array row
		...
		} else if strings.ContainsAny(row_year, "/") { //busca si contiene un /
			row_year = strings.ReplaceAll(row_year, "/", " ") //reemplaza en el string/cadena de texto de la variable
			//row_year, de un / a un espacio
		}
		if strings.ContainsAny(row_year, ",") { //Encontramos el index para posteriormente cortarlo
			findindex := strings.Index(row_year, ",")
			row_year = row_year[0:findindex]
		}

		//if row_year == year1 {
		//Con este cambio indicamos que nos ignore las mayúsculas/minúsculas del formato MMM (para que no sea case-sensitive)
		if strings.EqualFold(row_year, year1) {
			fmt.Printf("YEAR1 ROW: [%s]\n", row_year)
			row1 = row[1]
			row2 = row[2]
			row3 = row[3]
			row4 = row[4]
		} else if strings.EqualFold(row_year, year2) {
			...
			row3 = row[3]
			row4 = row[4]
		} else if strings.EqualFold(row_year, year3) {
			...
			row3 = row[3]
			row4 = row[4]
		} else if strings.EqualFold(row_year, year4) {
			...
			row3 = row[3]
			row4 = row[4]
		} else if strings.EqualFold(row_year, year5) {
			...
			fmt.Printf("AÑO/YEAR NO ENCONTRADO: [%s]\n", row_year)
		}
